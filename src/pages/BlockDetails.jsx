          ...
          {blockData.proposer && (
            <div className="grid grid-cols-1 md:grid-cols-3 py-4 px-6">
              ...
              <div className="md:col-span-2 mt-1 md:mt-0">
                <Link
                  to={`/account/${blockData.proposer}`}
                  className="text-primary hover:underline"
                >
                  {shortenAddress(blockData.proposer, 12)}
                </Link>
              </div>
            </div>
          )}
                ...
                <tbody className="bg-white dark:bg-gray-900 divide-y divide-gray-200 dark:divide-gray-800">
                  {currentTxs.map((tx, index) => (
                    ...
                    >
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        {(Number(tx.value) / 1000000000000000000).toFixed(4)}{" "}
                        REWA
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span
                          className={`px-2 py-1 text-xs rounded-full ${getTransactionStatusClass(
                            tx.status
                          )}`}
                        >
                          {tx.status}
